...
export function Dashboard() {
  ...
  const positions = holdings.map((h) => {
    ...
    return { ...h, quote: q, marketValue: mv, cost, pnl, pnlPct };
  });
  ...
  const dayPnl = positions.reduce((s, p) => s + p.quote.change * Number(p.quantity), 0);
  const dayPnlPct = equity === 0 ? 0 : (dayPnl / equity) * 100;

  const movers = topMovers();

  const historyData = candles("SPY", 30).map((c) => ({
    t: new Date(c.t * 1000).toLocaleDateString("en-US", { month: "short", day: "2-digit" }),
    v: c.c,
  }));
    ...
    <div className="mx-auto max-w-[1600px] space-y-6 p-6">
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        <Kpi
          ...
          value={loading ? undefined : formatCurrency(equity + (pf?.cash_balance ?? 0), pf?.base_currency)}
          hint={loading ? undefined : `${positions.length} positions`}
          icon={<Briefcase className="h-4 w-4" />}
        />
        <Kpi
          label="Today's P&L"
          value={loading ? undefined : formatCurrency(dayPnl, pf?.base_currency)}
          hint={loading ? undefined : formatPercent(dayPnlPct)}
          hintClass={pctClass(dayPnlPct)}
          trend={dayPnl >= 0 ? "up" : "down"}
          icon={<TrendingUp className="h-4 w-4" />}
        />
        <Kpi
          label="Total P&L"
          value={loading ? undefined : formatCurrency(totalPnl, pf?.base_currency)}
          hint={loading ? undefined : formatPercent(totalPnlPct)}
          hintClass={pctClass(totalPnlPct)}
          trend={totalPnl >= 0 ? "up" : "down"}
          icon={<Percent className="h-4 w-4" />}
        />
        <Kpi
          label="Buying power"
          value={loading ? undefined : formatCurrency(buyingPower, pf?.base_currency)}
          hint={loading ? undefined : `Cash ${formatCurrency(pf?.cash_balance ?? 0, pf?.base_currency)}`}
        ...
        <Card className="lg:col-span-2">
          <CardHeader className="flex-row items-center justify-between space-y-0">
            <div>
              ...
              <p className="text-xs text-muted-foreground">Last 30 days (SPY benchmark)</p>
            </div>
                      ...
                      <div key={p.id}>
                        <div className="flex items-center justify-between text-sm">
                          ...
                          <span className="numeric text-muted-foreground">
                            {formatCurrency(p.marketValue)} · {pct.toFixed(1)}%
                          </span>
                        </div>
                ...
                <TableBody>
                  {positions.map((p) => (
                    <TableRow key={p.id}>
                      ...
                      <TableCell className="numeric text-right">{formatCurrency(p.quote.price)}</TableCell>
  ...
  return (
    <Card>
      <CardContent className="p-5">
        ...
        <div className="mt-3 numeric text-2xl font-semibold tracking-tight">
...
function MoverList({ items }: { items: Quote[] }) {
  return (
    <div>
      {items.map((q) => (
        <div
        ...
        >
          ...
          <div className="text-right">
            ...
            <div className={`numeric text-xs ${pctClass(q.changePct)}`}>
              {formatPercent(q.changePct)} · vol {formatCompact(q.volume)}
            </div>
          </div>
        </div>
      ))}
...
function TickerTape() {
  const items = listUniverse().slice(0, 16);
  const seq = [...items, ...items];
  return (
    <div className="overflow-hidden rounded-md border border-border bg-surface-1">
      <div className="ticker-tape flex gap-8 py-2.5 pl-4 whitespace-nowrap">
        {seq.map((q, i) => (
          <div key={`${q.symbol}-${i}`} className="flex items-center gap-2 text-sm">
            ...
            <span className={`numeric text-xs ${pctClass(q.changePct)}`}>
