...
import { searchSymbols } from "@/lib/mock-market";
          ...
          {results.map((r) => (
            ...
            >
              ...
              <div className="flex flex-1 items-center gap-3">
                <span className="w-14 font-mono text-sm font-semibold">{r.symbol}</span>
                <span className="truncate text-sm text-muted-foreground">{r.name}</span>
              </div>
              <span className="numeric text-sm">{formatCurrency(r.price)}</span>
              <span className={`numeric ml-3 w-16 text-right text-xs ${pctClass(r.changePct)}`}>
