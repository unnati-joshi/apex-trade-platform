...
import { useQuotes, symbolMeta } from "@/lib/market";
...
export function PortfolioPage() {
  ...
  const uniqueSyms = Array.from(new Set(holdings.map((h) => h.symbol)));
  const { data: quotes = [] } = useQuotes(uniqueSyms);
  const priceOf = useMemo(() => new Map(quotes.map((q) => [q.symbol, q.price])), [quotes]);
  const px = (sym: string) => priceOf.get(sym) ?? 0;

  const longEquity = longs.reduce((s, h) => s + px(h.symbol) * Number(h.quantity), 0);
  const longCost = longs.reduce((s, h) => s + Number(h.avg_cost) * Number(h.quantity), 0);
  const shortLiability = shorts.reduce((s, h) => s + px(h.symbol) * Number(h.quantity), 0);
  ...
  const buyingPower = primary ? Number(primary.cash_balance) * Number(primary.margin_multiplier || 1) : totalCash;

  const sectors = longs.reduce<Record<string, number>>((acc, h) => {
    const meta = symbolMeta(h.symbol);
    const mv = px(h.symbol) * Number(h.quantity);
    acc[meta.sector] = (acc[meta.sector] ?? 0) + mv;
    return acc;
  }, {});
  ...
  const toggleMargin = useMutation({
    mutationFn: async () => {
      ...
      if (nextType === "cash" && shorts.length > 0) throw new Error("Close all short positions before switching to a cash account.");
    ...
    onSuccess: () => { toast.success("Account updated"); qc.invalidateQueries(); },
  ...
  return (
    <div className="mx-auto max-w-[1600px] space-y-6 p-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        {primary && (
          <Button variant="outline" size="sm" onClick={() => toggleMargin.mutate()} disabled={toggleMargin.isPending}>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
        <div className="space-y-6">
          <PositionsTable title="Long positions" rows={longs} loading={isLoading} side="long" priceOf={priceOf} />
          {shorts.length > 0 && <PositionsTable title="Short positions" rows={shorts} loading={isLoading} side="short" priceOf={priceOf} />}
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            {Object.keys(sectors).length === 0 && <div className="text-sm text-muted-foreground">No allocation yet.</div>}
            {portfolios.length > 0 && (
...
function PositionsTable({ title, rows, loading, side, priceOf }: {
  title: string; rows: Holding[]; loading: boolean; side: "long" | "short"; priceOf: Map<string, number>;
}) {
  return (
    <Card>
                  ...
                  <TableRow key={h.id}>
                    <TableCell>
                      <div className="flex items-center gap-2">
                        <span className={cn("grid h-6 w-6 place-items-center rounded text-[9px] font-bold", side === "short" ? "bg-bear/20 text-bear" : "bg-bull/20 text-bull")}>
                          {side === "short" ? "S" : "L"}
                        </span>
                        <div>
                          <div className="font-mono text-sm font-medium">{h.symbol}</div>
                          <div className="max-w-[240px] truncate text-xs text-muted-foreground">{meta.name}</div>
                        </div>
                      </div>
                    </TableCell>
                    ...
                    <TableCell className="numeric text-right">
                      <div className="inline-flex items-center gap-1">
                        {price > 0 ? (
                          <>
                            {(price - Number(h.avg_cost)) >= 0 ? <ArrowUpRight className="h-3 w-3 text-bull" /> : <ArrowDownRight className="h-3 w-3 text-bear" />}
